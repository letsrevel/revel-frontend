...
<div class={cn('space-y-6', className)}>
	...
	<section aria-labelledby="details-heading">
		<h2 id="details-heading" class="mb-3 text-xl font-semibold">Event details</h2>
		...
		<div class="grid gap-4 md:grid-cols-2">
			...
			<div class="flex gap-3 rounded-lg border bg-card p-4">
				...
				<div class="flex-1">
					<div class="text-sm font-medium text-muted-foreground">Date & Time</div>
					<div class="mt-1">
						...
						{#if event.end}
							<time datetime={event.end} class="block text-sm text-muted-foreground">
								Ends: {formatEventDate(event.end)}
							</time>
						{/if}
					</div>
				</div>
			...
			{#if capacityText}
				...
				>
					...
					<div class="flex-1">
						<div class="text-sm font-medium text-muted-foreground">Attendance</div>
						<div
			...
			{#if rsvpDeadlineText}
				...
				>
					...
					<div class="flex-1">
						<div class="text-sm font-medium text-muted-foreground">RSVP Deadline</div>
						<time
			...
			<div class="flex gap-3 rounded-lg border bg-card p-4">
				...
				<div class="flex-1">
					...
					{#if event.requires_ticket}
						<div class="mt-1 text-sm text-muted-foreground">Ticketed event</div>
					{:else}
						<div class="mt-1 text-sm text-muted-foreground">Free RSVP</div>
					{/if}
				</div>
			</div>
		</div>
