<script lang="ts">
	...
	const content = getLandingPage('it', 'kink-event-ticketing')!;

	// Generate breadcrumb structured data
	let breadcrumbData = $derived(
		generateBreadcrumbStructuredData([
			{ name: 'Home', url: page.url.origin },
			{ name: content.hero.headline, url: `${page.url.origin}/it/kink-event-ticketing` }
		])
	);
	...
	let webPageStructuredData = $derived({
		...
		url: `${page.url.origin}/it/kink-event-ticketing`,
		inLanguage: 'it',
		isPartOf: {
			'@type': 'WebSite',
			name: 'Revel',
...
<svelte:head>
	...
	<link rel="canonical" href="{page.url.origin}/it/kink-event-ticketing" />

	<!-- Open Graph -->
	<meta property="og:type" content="website" />
	<meta property="og:title" content={content.meta.title} />
	<meta property="og:description" content={content.meta.description} />
	<meta property="og:url" content="{page.url.origin}/it/kink-event-ticketing" />
