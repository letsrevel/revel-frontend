...
These need verification once PR 403 lands or merges its OpenAPI. If unresolved, follow-up backend issues will be filed before merging this frontend PR.

1. **No `GET /subscriptions/{id}/payments` endpoint in PR 397.** The drawer can only display payments observed via the create-payment mutation cache in the current session. Mitigation in Phase 1: accept empty payments on drawer open, append from successful `POST .../payments` responses, and treat the table as session-cached.
2. **`MySubscriptionSchema` returns `organization_id` only** — no name/slug/logo. Member listing page must batch-fetch org metadata via existing endpoints. Push backend to inline these fields.
3. **`Organization.membership_grace_period_days` and `Organization.membership_refund_policy` not exposed** in any read/update schema. No admin UI for these in Phase 1. Backend follow-up.
4. **`manage_subscriptions` permission flag** — verify it's surfaced on `OrganizationAdminDetailSchema` after `pnpm generate:api`. If not, file backend issue and fall back to staff/owner gate temporarily.
