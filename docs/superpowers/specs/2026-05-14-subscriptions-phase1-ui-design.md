...
Verified against `origin/dev/subscriptions` (PR 403) on 2026-05-14. Backend issue [#404](https://github.com/letsrevel/revel-backend/issues/404) tracked gaps #1 and #2; resolved by backend PR #405 (currently in pipeline, OpenAPI dumped locally on 2026-05-14).

1. **`GET /subscriptions/{sub_id}/payments` endpoint.** ✅ **Resolved by backend PR #405** — `organizationadminsubscriptionsListSubscriptionPayments` SDK function available after `pnpm generate:api`. Drawer fetches paginated payments on open. (Frontend PR merges after #405 lands on backend main.)
2. **`MySubscriptionSchema` org enrichment.** ✅ **Resolved by backend PR #405** — `organization_name`, `organization_slug`, `organization_logo_url` now inlined. No N+1 enrichment needed on member listing page.
...
### Updated data-flow consequences from PR #405

- **Drawer payments table** now uses a dedicated query key `['organization', slug, 'subscription', subId, 'payments']` against `listSubscriptionPayments` (paginated). No more session-cache fallback. Optimistic updates on record-payment insert into this query's cache.
- **Member listing** at `/account/memberships` renders directly from `MySubscriptionSchema` — no parallel org-detail queries. `OrgMembershipInline` likewise gets enough metadata from the single endpoint (org name/slug/logo are also available via SSR context on the org page itself).
